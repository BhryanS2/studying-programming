...
using namespace std;//uso espa√ßo d enomes standard
...
int ord(int a, int b){ // faco uma funÁ„o comum para que meu vetor seja ordenado do maior para o menor
...
int main(){ //declaro minha main
    ...
    for (int i = 0; i < n; i++) { // faÁo a soma de cada linha e cada coluna
        for (int j = 0; j < n; j++) {
            v[i] += m[i][j];
            k[j] += m[i][j];
        }
    ...
    for (int i = 0; i < n; i++) { //faÁo o peso de cada numero
        for (int j = 0; j < n; j++) {
            y = k[j] + v[i] - (2 * m[i][j]);
            r.push_back(y);
        }
    ...
    sort(r.begin(), r.end(), ord); // ordeno meu vetor r com base na funÁ„o ord
